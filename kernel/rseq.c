...
bool rseq_delay_resched(void)
{
	...
	if (__copy_from_user_inatomic(&flags, &t->rseq->flags, sizeof(flags)))
		return false;
	...
	if (__copy_to_user_inatomic(&t->rseq->flags, &flags, sizeof(flags)))
		return false;
...
void rseq_delay_schedule(void)
{
	...
	if (t->sched_time_delay) {
		...
		if (!t->rseq)
			return;
		if (__copy_from_user_inatomic(&flags, &t->rseq->flags,
                        sizeof(flags)))
                        return;
                flags |= RSEQ_CS_FLAG_RESCHEDULED;
                __copy_to_user_inatomic(&t->rseq->flags, &flags,
                        sizeof(flags));
	}
#endif
}
